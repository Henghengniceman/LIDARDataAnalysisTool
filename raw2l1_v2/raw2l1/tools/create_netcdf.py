...
def create_netcdf_global(conf, nc_id, data, logger):
    ...
    # altitude limits for background signal calculation are stored as attrs
    # (added manually on Jan22)
    if conf.has_option('reader_conf', 'bckgrd_min_alt'):
        BCK_MIN_ALT = int(conf.get('reader_conf', 'bckgrd_min_alt'))
    else:
        BCK_MIN_ALT = np.nan
    setattr(nc_id, 'BCK_MIN_ALT', BCK_MIN_ALT)
    if conf.has_option('reader_conf', 'bckgrd_max_alt'):
        BCK_MAX_ALT = int(conf.get('reader_conf', 'bckgrd_max_alt'))
    else:
        BCK_MAX_ALT = np.nan
    setattr(nc_id, 'BCK_MAX_ALT', BCK_MAX_ALT)
        ...
        if (data_type == 'string' and
           conf.get('conf', 'netcdf_format') == 'NETCDF3_CLASSIC'):
            ...
            try:
                ...
                msg = "107 Warning creating netCDF file '{}'".format(format(conf.get('conf', 'output')))
                msg += "key in %s does not exist in read data."
                logger.warning(msg % data_val)
            ...
            if option not in common.STRING_ATTR:
                if type(value) is str and KEY_READERDATA in value:
                    ...
                    try:
                        ...
                        mess = "key %s does not exist in read data."
...
def create_netcdf_variables(conf, data, nc_id, logger):
    ...
    for section in filter_conf_sections(conf, logger):
        ...
        # so we don't create the variable, but an attribute
        try:
            val_type = get_var_type(conf.get(section, 'type'), conf, logger)
            logger.debug("type " + repr(val_type))
        except configparser.NoOptionError:
            data_val = data[get_data_key(conf.get(var_name, 'value'))]
            setattr(nc_id, var_name, data_val)
            continue
        ...
        # Variable is created with the name "var_name" if the associated "data_key" exist
        data_val = conf.get(var_name, 'value')
        data_key = get_data_key(data_val)
        if data_key in data.keys():  # added on 2022/02/16. Only var_names with correspondence in data.keys() are created
            if dim == KEY_NODIM:
