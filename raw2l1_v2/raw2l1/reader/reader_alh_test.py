    ...
    for i_file, file_ in enumerate(list_files):
        ...
        if i_file == 0:
            ...
            if tmp != data_dim['range']:
                logger.critical('size of rcs range was %d in previous file and is now %d in %s',
        ...
        if i_file == 0:
            ...
            if tmp != data_dim['range_std']:
                logger.critical('size of std range was %d in previous file and is now %d in %s',
                                data_dim['range_std'], tmp, file_)
                sys.exit(1)
...
def init_data(data_dim, logger):
    ...
    for i_chan in range(data_dim['n_chan']):        
        ...
        data['std_{:02d}'.format(i_chan)] = np.ones((data_dim['time'], data_dim['range_std']),
                                                    dtype=np.float32) * MISSING_FLOAT                                                    
    ...
    data['laser_power_01'] = np.ones((data_dim['time'],),dtype=np.float32) * MISSING_FLOAT
    data['laser_power_02'] = np.ones((data_dim['time'],),dtype=np.float32) * MISSING_FLOAT
...
def read_header(file_id, data, data_dim, index, logger, date_only=False):
    ...
    current_range_resol_vector = data['range_resol_vect'][data['powermeter']==0] #those channels not powermeter 
